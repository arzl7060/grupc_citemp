
<div class="container-fluid text-center py-4"> <!-- Container utama -->
  <!-- Judul -->
  <div class="row justify-content-center mb-4">
    <div class="col-12">
      <h3 class="text-center bold"><b>Tim Capstone Project Kelompok C Kelas 14</b></h3>
    </div>
  </div>

  <!-- Card Profil -->
  <div class="row justify-content-center">
    <!-- Profil 1 -->
    <div class="col-auto mb-4 mx-2"> <!-- mx-2 untuk spacing horizontal -->
      <div class="card card-primary card-outline" style="width: 220px;">
        <div class="card-body text-center">
          <img class="img-fluid rounded-circle mb-3" 
               src="<?= base_url('AdminLTE') ?>/dist/img/logoc-food.png"
               style="width: 128px; height: 128px; object-fit: cover;">
          <h4>Ahmad Rizal</h4>
          <br>
          <p class="text-secondary">043070108</p>
        </div>
      </div>
    </div>

    <!-- Profil 2 -->
    <div class="col-auto mb-4 mx-2">
      <div class="card card-primary card-outline" style="width: 220px;">
        <div class="card-body text-center">
          <img class="img-fluid rounded-circle mb-3" 
               src="<?= base_url('AdminLTE') ?>/dist/img/logoc-food.png"
               style="width: 128px; height: 128px; object-fit: cover;">
          <h4>Cecillia Dwi Putri</h4>
          <br>
          <p class="text-secondary">043682183</p>
        </div>
      </div>
    </div>

    <!-- Profil 3 -->
    <div class="col-auto mb-4 mx-2">
      <div class="card card-primary card-outline" style="width: 220px;">
        <div class="card-body text-center">
          <img class="img-fluid rounded-circle mb-3" 
               src="<?= base_url('AdminLTE') ?>/dist/img/logoc-food.png"
               style="width: 128px; height: 128px; object-fit: cover;">
          <h4>Bayu Agung Nugraha</h4>
          <p class="text-secondary">042168627</p>
        </div>
      </div>
    </div>

    <!-- Profil 4 -->
    <div class="col-auto mb-4 mx-2">
      <div class="card card-primary card-outline" style="width: 220px;">
        <div class="card-body text-center">
          <img class="img-fluid rounded-circle mb-3" 
               src="<?= base_url('AdminLTE') ?>/dist/img/logoc-food.png"
               style="width: 128px; height: 128px; object-fit: cover;">
          <h4>Reyhan Kamil Ikhtiawan</h4>
          <p class="text-secondary">044312517</p>
        </div>
      </div>
    </div>

    <!-- Profil 5 -->
    <div class="col-auto mb-4 mx-2">
      <div class="card card-primary card-outline" style="width: 220px;">
        <div class="card-body text-center">
          <img class="img-fluid rounded-circle mb-3" 
               src="<?= base_url('AdminLTE') ?>/dist/img/logoc-food.png"
               style="width: 128px; height: 128px; object-fit: cover;">
          <h4>Rizieq Fadillah</h4>
          <br>
          <p class="text-secondary">051375001</p>
        </div>
      </div>
    </div>
  </div>
</div>
